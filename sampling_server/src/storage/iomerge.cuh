...
struct IOReq{
    uint64_t ssd_addr;
    uint64_t gpu_addr[Max_Merge_Lenth];//void **gpu_addr;
    uint64_t lenth;
    ...
    __forceinline__
    __host__ __device__
    IOReq(uint64_t ssd,uint64_t lenth){
        ssd_addr=ssd;
        this->lenth=lenth;
        // gpu_addr=new uint64_t[lenth];
        for(int i=0;i<lenth;i++){
            gpu_addr[i]=my_ULLONG_MAX;
        }
    }

    __host__ __device__
    bool operator < (const IOReq& lhs)const{
        return this->ssd_addr<lhs.ssd_addr;
    }
...
__global__ 
void print(cache_id *D_array,uint64_t len){
...
__global__
void get_miss(cache_id *D_ssd_plus_buffer,uint32_t *cache_miss,uint64_t *H_ids,uint64_t buffer_cnt);
...
struct IOMerge{
    ...
    uint64_t typesize{512};
    uint64_t buffer_cnt;
    
    cache_id* D_ssd_plus_buffer{nullptr};
    size_t temp_storage_bytes{1024000};
    void *d_temp_storage{nullptr};
    // uint32_t *D_cache_miss{nullptr};
    // uint64_t *D_ids{nullptr};
    uint64_t* D_split_flag{nullptr};
    IOReq *D_ret{nullptr};
    uint64_t *D_ret_ssd{nullptr};

    // __forceinline__
    __host__ __device__
    IOMerge(void *ssd,void *gpu,uint64_t grid_size,uint64_t block_size,uint64_t merge_lenth,uint64_t typesize,uint64_t buffer_cnt,size_t temp_storage_bytes):
    ...
    // __forceinline__
    __host__ __device__
    IOMerge(void *ssd,void *gpu,uint64_t grid_size,uint64_t block_size,uint64_t merge_lenth,uint64_t typesize,uint64_t buffer_cnt):
                    ssd_start_addr(ssd),gpu_start_addr(gpu),grid_size(grid_size),block_size(block_size),merge_lenth(merge_lenth),
    ...
    __host__
    IOReq* do_io_merge(uint32_t *D_cache_miss,uint64_t *D_ids,cudaStream_t stream){
        ...
        // print_Dret_ssd<<<1,1>>>(D_ret_ssd,miss_cnt,ssd_start_addr);
        // print_Dret<<<1,1>>>(D_ret,miss_cnt,ssd_start_addr,gpu_start_addr);
...
void merge_kernel(cache_id *D_ssd_plus_buffer,IOReq *D_ret,uint64_t *D_ret_ssd,uint64_t* D_split_flag,uint64_t lenth,
    ...
    for(uint64_t i=thread_id;i<lenth;i+=blockDim.x*gridDim.x){
        ...
        if(D_split_flag[i]==1){
            ...
            uint64_t start=i+1;
            req.gpu_addr[D_ssd_plus_buffer[i].ssd_id-base_id]=(uint64_t)gpu_start_addr+typesize*D_ssd_plus_buffer[i].buffer_id;
            while(D_ssd_plus_buffer[start].ssd_id!=my_ULLONG_MAX&&D_split_flag[start]==0){
                req.gpu_addr[D_ssd_plus_buffer[start].ssd_id-base_id]=(uint64_t)gpu_start_addr+typesize*D_ssd_plus_buffer[start].buffer_id;
                start++;
            }
            D_ret[i]=req;
            // printf("cache:%lu\n",((uint64_t)D_ret[i].ssd_addr-(uint64_t)ssd_start_addr)/typesize);
